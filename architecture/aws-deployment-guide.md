...
2. Upload the contents of `backend/static/` into this bucket:
   ```bash
   aws s3 sync backend/static/ s3://yourdomain-assets/static/ --exclude "_thumbcache/*"
   ```
   > The `_thumbcache/` folder is local PIL cache — do NOT upload it.
...
1. EC2 → Launch Instance
   ...
   - **Instance type:** `t3.small` (2 vCPU, 2GB — enough for PIL thumbnails; upgrade if needed)
...
server {
    ...
    location / {
        ...
        # Increase timeouts for thumbnail generation (PIL can be slow on first call)
...
### STEP 7 — Wire Up Assets CDN in the Backend

Right now the backend serves original images at `/static/images/...` and generates thumbnails at `/thumb/...`. With assets on S3:

- **Original images** → redirect or repoint to `https://assets.yourdomain.com/static/images/...`
- **Thumbnails** → still generated by EC2's `/thumb/...` endpoint (PIL must run server-side)
- **PDFs** → redirect to `https://assets.yourdomain.com/static/technical_specification_pdfs/...`

**Two options (we'll decide together which to implement):**

**Option A — Backend redirects (minimal frontend change):**
Add 301 redirects in `server.py` for `/static/` routes → CDN URLs. Thumbnails stay as-is.

**Option B — Frontend references CDN directly (cleanest):**
Update `REACT_APP_ASSETS_URL` env var; update asset URL construction in the frontend to use it for image/PDF links, keep `REACT_APP_BACKEND_URL` for API calls and `/thumb/` calls.

We'll implement this together in Step 7's deep-dive.
...
8.5 **Rate Limiting** (optional at launch) — Add `limit_req` in Nginx for `/thumb/` (PIL is CPU-heavy)  
